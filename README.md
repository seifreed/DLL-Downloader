...
A CycloneDX 1.6 SBOM for the runtime dependency closure is committed at
[`sbom.cdx.json`](sbom.cdx.json) and regenerated by:

```bash
python scripts/generate_sbom.py -o sbom.cdx.json
```

It documents the application and its runtime dependencies (not the
development/test tooling) with full supplier, license, hash, and
source/distribution provenance. The SBOM scores **10.0/10.0 Grade A** on
every NTIA and BSI TR-03183-2 profile of [sbomqs](https://github.com/interlynk-io/sbomqs);
CI enforces this contract via `scripts/check_sbom_quality.py`:
